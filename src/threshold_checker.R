...
for(threshold in 4){
  ...
  df_suggestions <- rbind(df_suggestions, cbind(threshold,df_suggestions_i))
  ...
  source("benchmark.R")
  benchmark_results_i <- benchmark(predict_stupid_backoff_DT, 
            3,
            # additional parameters to be passed to the prediction function can be inserted here
            sent.list = list('tweets' = tweets, 
                             'blogs' = blogs), 
            ext.output = F)
  benchmark_results <- rbind(benchmark_results, cbind(threshold, benchmark_results_i))
}
